/**
 * RiemannControls — sub-interval slider + sample-point pill toggle.
 * Collapsible panel. Debounces API calls 400 ms after slider stops.
 */
...
export default function RiemannControls({ exprStr, bounds = [0, 4], onRectangles, onSumResult }) {
  ...
  const handleSliderChange = (e) => {
    ...
    debounceRef.current = setTimeout(() => fetchRiemann(val, samplePoint), 400)
  }

  const handleSampleChange = (sp) => {
    setSamplePoint(sp)
    fetchRiemann(n, sp)
  }

  const errorAbs = sumData?.exact_value != null
    ? Math.abs(sumData.riemann_sum - sumData.exact_value)
    : null
        ...
        <div className={styles.body}>
          ...
          <div className={styles.sliderRow}>
            <div className={styles.sliderHeader}>
              <span className={styles.label}>Sub-intervals</span>
              <span className={styles.nValue} aria-live="polite">{n}</span>
            </div>
            <input
              id="n-slider"
              type="range"
          ...
          <div className={styles.sampleRow}>
            <span className={styles.label}>Sample point</span>
          ...
          {loading && <LoadingBar active label="Computing Riemann sum…" />}
          {error && <div className={styles.error} role="alert">{error}</div>}

          {sumData && !loading && (
            <div className={styles.results}>
              <div className={styles.resultRow}>
                <span className={styles.resultLabel}>Riemann sum (n={n})</span>
                <span className={styles.resultValue}>{sumData.riemann_sum?.toFixed(6)}</span>
              </div>
              {sumData.exact_value != null && (
                <>
                  <div className={styles.resultRow}>
                    <span className={styles.resultLabel}>Exact value</span>
                    <span className={styles.resultValue}>{sumData.exact_value?.toFixed(6)}</span>
                  </div>
                  <div className={styles.resultRow}>
